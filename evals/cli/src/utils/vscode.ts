...
`,
	...
	const settings = {
		...
		"workbench.view.extension.busterwood.clinesp-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.busterwood.clinesp-ActivityBar",
	...
	const args = [
		...
		"--command",
		"workbench.view.extension.busterwood.clinesp-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.busterwood.clinesp-ActivityBar');
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						const extension = vscode.extensions.getExtension('busterwood.clinesp');
						if (!extension) {
						...
						await vscode.commands.executeCommand('workbench.view.extension.busterwood.clinesp-ActivityBar');
